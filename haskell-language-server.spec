# generated by cabal-rpm-2.3.3 --stream hackage --standalone
...
cabal-tweak-flag plugin-tutorial False

%if %[v"%{ghc_major}" == v"9.12"]
...
%if %[v"%{ghc_version}" > v"9.14"]
# https://github.com/phadej/boring/issues/48
%define allow_newer --allow-newer
%endif
%if 0%{?fedora} < 44
%define constraints --constraint 'extra >= 1.8'
%endif
cabal install %{!?_with_compiler_default:-w ghc-%{ghc_version}} --install-method=copy --enable-executable-stripping --installdir=%{buildroot}%{_bindir} %{?allow_newer} %{?constraints}
...
%changelog
* Mon May 04 2026 Jens Petersen <petersen@redhat.com> - 2.14.0.0-1
